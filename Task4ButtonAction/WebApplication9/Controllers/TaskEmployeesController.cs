...
namespace WebApplication9.Controllers
{
    public class TaskEmployeesController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,First_Name,Last_Name,Email,Phone,Age,Job_Title,Gender")] TaskEmployee taskEmployee, HttpPostedFileBase image, HttpPostedFileBase cv, int? id)
        {
            if (ModelState.IsValid)

            {
                var customer = db.TaskEmployees.AsNoTracking().FirstOrDefault(x => x.Id == id);
                if (image != null)
                ...
                {
                    taskEmployee.image = customer.image;
                }
